...
QUESTIONS["annotate"] = {
    "prompt": (
        "Analyze this traffic camera image and classify emergency vehicles and incident type.\n\n"
        "\u2550\u2550\u2550 PART 1 \u2014 EMERGENCY VEHICLES \u2550\u2550\u2550\n\n"
        "Step 1: Assess image clarity. Choose HARD TO TELL if any of these apply:\n"
        "  \u2022 Flashing lights wash out or obscure the scene\n"
        "  \u2022 The image is dark, foggy, or motion-blurred\n"
        "  \u2022 Vehicles are far enough away that you cannot read their markings\n"
        "  \u2022 You can see lights or a vehicle silhouette but cannot confirm the type\n"
        "  \u2022 You have any meaningful doubt about what vehicle type is present\n\n"
        "Hard to tell is the correct answer whenever you are not confident. "
        "Default to it rather than guessing.\n\n"
        "Step 2 (only if you are confident): Count each of the following vehicle types visible:\n"
        "  - POLICE vehicles (marked patrol cars, police SUVs, highway patrol)\n"
        "  - FIRE TRUCKS (fire engines, ladder trucks, heavy rescue)\n"
        "  - AMBULANCES (EMS units, paramedic vehicles)\n\n"
        "  For each type present, select singular (exactly 1) or plural (2 or more).\n"
        "  - Do NOT count the same vehicle twice; do NOT count light reflections or light bars as vehicles.\n"
        "  - If unsure between 1 or 2, choose singular.\n"
        "  - 'No emergency vehicles' = you are certain none of the above are present anywhere in the frame.\n"
        "  - 'Hard to tell' and 'No emergency vehicles' are mutually exclusive and each overrides "
        "all vehicle-type selections.\n\n"
        "\u2550\u2550\u2550 PART 2 \u2014 INCIDENT TYPE \u2550\u2550\u2550\n\n"
        "Work through this numbered checklist in order. Apply every label that clearly matches.\n\n"
        "1. CRASH \u2014 select ONLY if you can directly see physical collision damage: crumpled or "
        "deformed bodywork, deployed airbags, a vehicle resting sideways or in a ditch, or large "
        "debris on the road surface from the impact. Emergency vehicles at the scene are NOT "
        "evidence of a crash on their own. No clearly visible damage = do NOT select.\n\n"
        "2. PULLED OVER \u2014 select ONLY if ALL THREE conditions are simultaneously visible:\n"
        "   (a) a civilian vehicle is stopped fully off the travel lanes, on the road shoulder or verge;\n"
        "   (b) a police vehicle is parked directly behind or beside that civilian vehicle;\n"
        "   (c) both vehicles are stationary.\n"
        "   If you cannot confirm all three conditions, do NOT select. A police car at an "
        "intersection, moving in traffic, or responding to another incident is NOT a traffic stop.\n\n"
        "3. BLOCKED ROAD \u2014 select ONLY if you can directly see what is physically preventing "
        "all traffic from passing through a lane:\n"
        "   \u2022 Cones or solid barriers visibly spanning the complete width of a lane from edge to edge\n"
        "   \u2022 A vehicle, wreckage, or debris occupying the full lane width with no gap for traffic\n"
        "   The physical obstruction itself must be visible. You CANNOT infer blockage from: emergency "
        "vehicle presence, flashing lights, traffic appearing slow, or the road looking quiet. If you "
        "are reasoning 'there must be a blockage because police are here' or 'the road looks blocked' "
        "without seeing what is physically blocking it \u2014 do NOT select. A police car or fire "
        "truck at the roadside, on the shoulder, or at an intersection is NOT blocking the road.\n\n"
        "4. CONSTRUCTION \u2014 select ONLY if you can see active road surface construction work:\n"
        "   \u2022 Large dedicated construction machinery (excavators, asphalt pavers, concrete rollers, "
        "road graders) actively operating on the road surface \u2014 not merely parked\n"
        "   \u2022 Workers actively breaking or repairing the road surface itself\n"
        "   IMPORTANT: Police officers, firefighters, and EMS personnel routinely wear hi-vis "
        "vests \u2014 their presence does NOT indicate construction. Fire trucks, police vehicles, tow "
        "trucks, and utility vans are NOT construction machinery even if yellow or orange. An "
        "emergency response scene with cones is NOT construction.\n\n"
        "5. FIRE \u2014 select if you can see actual flames or heavy, thick smoke rising from within "
        "the scene.\n\n"
        "6. CROWD \u2014 select if a group of civilians (not uniformed emergency responders) is "
        "visibly gathered in or near the roadway.\n\n"
        "7. IDENTIFIABLE INCIDENT, NOT ONE OF THE ABOVE \u2014 select when ALL of these are true:\n"
        "   (a) there is clear evidence of an active emergency response: multiple emergency vehicles "
        "present, or personnel visibly deployed outside vehicles, or an obviously active scene;\n"
        "   (b) NONE of items 1\u20136 apply.\n"
        "   Do NOT select this alongside any of 1\u20136 \u2014 it is exclusive with all of them.\n"
        "   Do NOT select this for a single parked police or emergency vehicle showing no signs of "
        "active deployment \u2014 select 8 instead.\n\n"
        "8. NO INCIDENT CLEARLY VISIBLE \u2014 select when no active emergency response is visible. "
        "This includes: no emergency vehicles present; a single emergency vehicle parked or "
        "stationary with no crew deployed and no obvious active scene; or normal traffic flow. "
        "Do NOT select if multiple emergency vehicles are present or an active scene is visible.\n\n"
        "1\u20136 can be combined freely. 7 and 8 are mutually exclusive with each other and with 1\u20136."
    ),
    "tool": {
        "type": "function",
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "hard_to_tell":          {"type": "boolean", "description": "Cannot confidently identify vehicle type — exclusive, default when uncertain"},
                    "crash":                 {"type": "boolean", "description": "A crash or collision is visible"},
                    "pulled_over":           {"type": "boolean", "description": "A pulled-over vehicle is visible"},
